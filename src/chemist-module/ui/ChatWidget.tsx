                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar relative">
                            ...
                            <div className="flex flex-col space-y-6 relative z-10">
                                {messages.map((msg, index) => {
                                    const hasOrderTrigger = msg.role === 'assistant' && msg.content.includes('#open-po');
                                    const displayContent = hasOrderTrigger 
                                        ? msg.content.replace(/\[([^\]]+)\]\(#open-po\)/g, '').trim()
                                        : msg.content;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                {hasOrderTrigger && index === messages.length - 1 && messages.length > 3 && (
                                                    <div className="mt-4 pt-3 border-t border-slate-100/50">
                                                        <button
                                                            onClick={(() => setIsPOFormOpen(true))}
                                                            className="inline-flex flex-row items-center gap-2 px-3.5 py-2 rounded-full bg-slate-50 text-slate-700 hover:bg-cyan-50 hover:text-cyan-700 hover:border-cyan-200 border border-slate-200 text-xs font-semibold shadow-sm transition-all group/btn"
                                                        >
                                                            <Icon icon="ph:shopping-cart-bold" className="h-3.5 w-3.5 text-slate-400 group-hover/btn:text-cyan-600" />
                                                            Start Order
                                                            <Icon icon="ph:arrow-right-bold" className="h-3 w-3 opacity-50 group-hover/btn:opacity-100 group-hover/btn:translate-x-0.5 transition-all text-cyan-600" />
                                                        </button>
                                                    </div>
                                                )}
                                            </div>
