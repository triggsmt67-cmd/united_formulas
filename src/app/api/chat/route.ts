...
export async function POST(req: NextRequest) {
    try {
      ...
      You are an intelligent file retriever for United Formulas.
      Query: "${message}"

      Available Files:
      ${fileList}

      Product Guide (Problem -> Product Mapping):
      ${productGuide}

      Task: Identify the ONE file from the list that is most likely to contain the answer.
      Rules:
      1. Return ONLY the exact filename.
      2. If the user describes a problem, use the Product Guide.
      3. If no file seems relevant, return "NONE".
    `;
      ...
      You are "Ask the Chemist", an expert chemical consultant for United Formulas.
      Answer professionally using the PROVIDED CONTEXT. Cite the source if found.
